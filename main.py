...
def print_error(output, err=False, verbose=False):
    '''
    ...
    '''
    if verbose or err:
        print('{}: {}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3], output), file=stderr)
...
def get_oci_secret(oci_config, oic_secret_ocid):
    ...
    print_error('Retrieved Secret', verbose=args.verbose)
...
if __name__ == '__main__':
    ...
    with open(args.configFile, 'r') as cf:
        config = load(cf)
    print_error('Loaded config', verbose=args.verbose)

    # Initialize OCI config
    oci_config = init_oci()
    print_error('Initialized OCI', verbose=args.verbose)

    # Initialize JWT or Basic Auth based on presence of JWT_AUTH_SECRET_FIELD
    if JWT_AUTH_SECRET_FIELD in config:
        secret = get_oci_secret(oci_config, config[JWT_AUTH_SECRET_FIELD])
        auth = JWTAuth(secret)
        auth_method = 'JWT'
    else:
        oic_credentials = get_oci_secret(oci_config, config[BASIC_AUTH_SECRET_FIELD])
        auth = (oic_credentials[USERNAME_FIELD], oic_credentials[PASSWORD_FIELD])
        auth_method = 'Basic'
    print_error('Created authorization', verbose=args.verbose)

    tests = []
    print_error('Starting {} iterations'.format(config[ITERATIONS_FIELD]), verbose=args.verbose)
    ...
    for cnt in range(config[ITERATIONS_FIELD]):
        ...
        for req in config[REQUESTS_FIELD]:
            entry = {'Auth': auth_method, NAME_FIELD: req[NAME_FIELD]}
            url = config[BASE_URL_FIELD] + req[PATH_FIELD]
            start = time()
            if req[VERB_FIELD] == 'GET':
                resp = get(config[BASE_URL_FIELD] + req[PATH_FIELD], auth=auth, params=req[PARAMS_FIELD])
            elif req[VERB_FIELD] == 'POST':
                resp = post(config[BASE_URL_FIELD] + req[PATH_FIELD], auth=auth, data=req[PARAMS_FIELD])
            else:
                resp = None
            elapsed = time() - start
            ...
            for field in req[ELAPSED_FIELDS_FIELD]:
                ...
                if len(result) > 0:
                    ...
                    print_error('Missing response field {}: {}'.format(field[ELAPSED_PATH_FIELD], resp), err=True)
            tests.append(entry)
